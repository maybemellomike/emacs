...
~use-package~ is built into Emacs 29+. This block ensures the package archives
are set up and that ~use-package~ will auto-install any package declared with
~:ensure t~, replacing the manual ~dolist~ install loop from before.
...
;; Always refresh the package list on startup so MELPA version
;; hashes never go stale and cause "Not found" install errors.
(package-refresh-contents)
