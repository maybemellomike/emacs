...
~jinx~ is a fast spell checker backed by the system ~enchant-2~ library.

- ~C-c s~ toggles ~jinx-mode~ on and off in the current buffer
- ~]s~ / ~[s~ jump to next/previous misspelling (mirrors Vim)
- ~C-c $~ opens a correction menu at point

On NixOS: ~environment.systemPackages = [ pkgs.enchant pkgs.hunspellDicts.en-us ];~
