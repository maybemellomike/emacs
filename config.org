...
This gets rid of some of the built in GUI elements like the menu bar, tool bar, and scroll bar which are, well ugly.
...
The two keys to a stable gutter width are:
1. Set ~display-line-numbers-width~ globally as a default BEFORE the mode enables.
2. In org-mode, re-assert it AFTER ~variable-pitch-mode~ fires, because proportional
   fonts cause Emacs to recalculate gutter metrics. We do this by appending to the
   hook (the ~t~ argument to ~add-hook~) so it runs last.

 For some reason this issue of the changing indentiation is still persistant in Org Mode documents but it works elsewhere. 
...
I don't know how anyone could Emacs themselves without this snippit right here.
...
Simple and this actually appears to work.
...
In LaTeX files and Typst files I can M-x zenmode and this will do what the Vim plugin does within the frame showing the document. 
#+BEGIN_SRC emacs-lisp
...
Seems to work alright for the time being making things more readable. 
