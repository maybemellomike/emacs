...
- [[#spacemacs-like-keybinds][Spacemacs like Keybinds]]
  ...
  - [[#music-player-emms][Music Player (EMMS)]]
...
* Performance
There are different settings I've come across which help with performace. Usually in terms of changing the order in which things are loaded or rendering fonts in some spots and not others.
...
#+BEGIN_SRC emacs-lisp
(set-face-attribute 'default nil        :family "Aporetic Serif Mono"     :height 120)
(set-face-attribute 'fixed-pitch nil    :family "Aporetic Serif Mono"     :height 120)
(set-face-attribute 'variable-pitch nil :family "CMU Sans Serif" :height 140)
#+END_SRC
...
A built-in music player using EMMS with mpv as the backend. Opens a
two-pane layout with the browser on the left and playlist on the right,
similar to CMUS. Album art is displayed in a floating buffer when
available.
