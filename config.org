...
*** Kill All Hugo Servers (~C-c o k~)

Kills every background process whose name starts with ~hugo:~, regardless of
which project it belongs to. Useful for a clean shutdown when you are done
working across multiple projects and don't want to stop them individually.

#+BEGIN_SRC emacs-lisp
(defun my/stop-all-hugo ()
  ...
  (let ((killed 0))
    ...
    (if (zerop killed)
      ...
      (message "Stopped %d hugo server%s." killed (if (= killed 1) "" "s")))))

(global-set-key (kbd "C-c o k") #'my/stop-all-hugo)
#+END_SRC
