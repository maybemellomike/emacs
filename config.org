...
~flyspell~ is a spell checker built into Emacs — no compilation or external
Emacs packages required. It delegates to whichever spell program is available
on the system (~hunspell~, ~aspell~, or ~ispell~). On NixOS add ~pkgs.hunspell~
and ~pkgs.hunspellDicts.en-us~ to your system packages; on most other distros
hunspell is available via the package manager.

~flyspell-mode~ underlines misspelled words as you type in text buffers.
~flyspell-prog-mode~ checks only comments and strings in code buffers.

- ~C-c s~ toggles ~flyspell-mode~ in the current buffer
- ~C-c $~ opens a correction menu at point
- ~]s~ / ~[s~ jump to next/previous misspelling (mirrors Vim ~]s~ / ~[s~)
...
~C-c p t~ opens the preview, ~C-c p T~ stops it.
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun my/org-typst-preview-update ()
    "Re-export the current org buffer to Typst and refresh the pdf-tools window.
  Runs on `after-save-hook' (buffer-local)."
    (when my/org-typst-preview-active
      (org-export-to-file 'typst my/org-typst-preview--typ-path)
      (run-with-timer 1.0 nil
                      #'my/org-typst-preview--do-revert
  ...
  (defun my/org-typst-preview--open-split (org-buf pdf-path proc-name)
    ...
    (if (not (file-exists-p pdf-path))
      ...
      (let* ((org-win (get-buffer-window org-buf))
        ...
        (message "Org→Typst preview active — saving will auto-refresh."))))
...
#+BEGIN_SRC emacs-lisp
  ;; Terminal
  (evil-define-key 'normal 'global (kbd "<leader>tt") #'my/toggle-eshell-bottom)

  ;; Files
  (evil-define-key 'normal 'global (kbd "<leader>ff") #'find-file)
  (evil-define-key 'normal 'global (kbd "<leader>fd") #'my/open-config)
#+END_SRC
...
(defun my/sync-dotfiles ()
  ...
  (let ((msg (read-string "Commit message: ")))
    (if (string-empty-p msg)
        (message "Sync aborted — commit message cannot be empty.")
      (let* ((default-directory (expand-file-name "~/dox/nixdots/")))
        (shell-command (format "cp -r %s %s"
                               (expand-file-name "~/dots/")
                               default-directory))
        (shell-command "git add .")
        (shell-command (format "git commit -m %S" msg))
        (shell-command "git push")
